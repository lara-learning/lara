...
export const TraineePaperPage: React.FC = () => {
  ...
  return (
    <Template type="Main">
      {loading && <Loader/>}
      {!loading && data.trainees.map((trainee) => (
        <div key={trainee?.id}>
              ...
              paper?.traineeId == currentUser.id ? (
                <Spacer bottom="xl" key={paper?.id}>
                  <Container overflow={'visible'} padding={'l'}>
                    <Box width={[3, 5 / 5]}>
                      <H1
                        center>{strings.paper.dashboard.title + ' ' + paper?.client + ' - ' + paper?.subject}</H1>
                      <Spacer bottom="xl">
                        <Text
                          size={'copy'}>{strings.paper.dashboard.description}</Text>
                      </Spacer>
            ...
            <Flex alignItems={'center'} flexDirection={'column'}>
              <Box width={[1, 3 / 5]}>
                ...
                <Spacer y="l">
                  <Flex alignItems={'center'} flexDirection={'column'}>
                    <PrimaryButton
                      disabled={true}>{strings.paper.empty.createBriefing}</PrimaryButton>
                  </Flex>
                </Spacer>
                <EmptyPaper/>
              </Box>
