...
type UseDayHelper = {
  isValidTimeUpdate: (day: Pick<Day, 'status'> & { entries: Pick<Entry, 'time'>[] }, newTime: number) => boolean
  getTotalMinutes: (day: Pick<Day, 'status'> & { entries: Pick<Entry, 'time'>[] }) => number
}
